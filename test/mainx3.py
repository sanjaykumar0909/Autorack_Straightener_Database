...
get_file_ps1 = "C:\\Users\\2003s\\PycharmProjects\\AutoRackCSVs\\get_file.ps1"
...
while True:
    ...
    if fetch_choice not in range(1, 6):
        ...
        command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", get_serial_ps1,
                   "-file_name", file_name]
        ...
        print("Enter serial number of the file")
        serial_num = input().strip().split(".")

        command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", get_file_ps1,
                   "-months_elapsed_str", serial_num[0], "-ser_num_count_str", serial_num[1]]
        process = Sprc.Popen(command, stdout=Sprc.PIPE, stderr=Sprc.PIPE, text=True)
        op, err = process.communicate()
        ret_code = process.returncode
        op = op.strip()
        ...
        if ret_code==0:
            if op == "-1" or op == "-2":
                print("No file found for this serial number")
            else:
                print(f"File name of the serial number is : {op}")
        ...
        if t1 != None:
            ...
            if t2 != None:
                ...
                command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", fetch_by_time_ps1, "-folder_path",
                           csv_folder,
